#!/bin/bash
# Install the Discord plugin into an Agent Zero instance.
#
# Usage:
...
PLUGIN_DIR="$A0_ROOT/usr/plugins/discord"

echo "=== Discord Plugin Installer ==="
...
# Create symlink so 'from plugins.discord.helpers...' imports work
SYMLINK="$A0_ROOT/plugins/discord"
if [ ! -e "$SYMLINK" ]; then
...
if [ "$A0_ROOT" = "/a0" ] && [ -d "/git/agent-zero/usr" ]; then
    GIT_PLUGIN="/git/agent-zero/usr/plugins/discord"
...
echo ""
echo "Next steps:"
echo "  1. Configure your bot token in the Discord plugin settings (WebUI)"
echo "     Or set DISCORD_BOT_TOKEN environment variable"
echo "  2. Restart Agent Zero to load the plugin"
echo "  3. Ask the agent: 'List Discord channels in server <guild_id>'"
